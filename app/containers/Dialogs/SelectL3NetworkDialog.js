...
import {
  updateDbL3NetworkList
} from 'containers/App/dbActions';

import { selectDbL3Network } from 'containers/App/selectors';

import appStyles from 'containers/App/styles.css';

class SelectL3NetworkDialog extends Component {
    ...
    apiCall({
      ...
      if (ret.success) {
        if (!! ret.inventories && ret.inventories.length > 0) {
          self.props.updateDbL3NetworkList(ret.inventories);
          let pageItemList = [];
  ...
  render() {
    ...
    var list = [];
    let dbL3Network = this.props.dbL3Network;
    if (!!dbL3Network && !!windowData.list) {
      for (var i in windowData.list) {
        if (!!dbL3Network[windowData.list[i].uuid]) {
          list.push(Object.assign({}, dbL3Network[windowData.list[i].uuid], {'selected': windowData.list[i].selected}));
        }
      }
    }
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    updateDbL3NetworkList: (list) => dispatch(updateDbL3NetworkList(list))
  };
}
...
export default connect(mapStateToProps, mapDispatchToProps)(SelectL3NetworkDialog);